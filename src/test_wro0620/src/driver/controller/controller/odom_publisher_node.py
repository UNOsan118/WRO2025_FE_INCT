...
class Controller(Node):
    ...
    def cmd_vel_callback(self, msg):
        if self.machine_type == 'MentorPi_Mecanum':
            ...
            # Call the new class method for the modified robot
            speeds = self.ackermann_original.set_velocity(self.linear_x, self.angular_z)

            # speeds[1] is the motor command
            self.motor_pub.publish(speeds[1])
            
            # speeds[0] is the servo command
            if speeds[0] is not None:
                servo_state = PWMServoState()
                servo_state.id = [3] # Assuming servo ID is 3
                servo_state.position = [int(speeds[0])]
                data = SetPWMServoState()
                data.state = [servo_state]
                data.duration = 0.02
